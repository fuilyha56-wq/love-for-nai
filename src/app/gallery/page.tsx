...
import { ArrowLeft, Eye, EyeOff, Heart, Image as ImageIcon, Images, Link2, RotateCcw, Send } from "lucide-react";
...
export default function GalleryPage() {
  ...
  async function copyImageLink(item: GalleryItem) {
    const url = `${window.location.origin}${item.imageUrl}`;
    setMessage((await writeClipboard(url)) ? `图片直链已复制：${url}` : `图片直链：${url}`);
  }
  return (
    <main className="min-h-screen bg-[var(--paper)] text-[var(--ink)]">
      ...
      <section className="mx-auto max-w-7xl p-4 sm:p-7">
        ...
        {items.length ? <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{items.map((item) => {
          ...
          return (
          <article key={item.id} className="overflow-hidden rounded-md border border-[var(--line)] bg-white">
            <div className="relative aspect-[4/5] bg-[#ebe9e2]">
              ...
              {isHidden && (
                <button type="button" onClick={() => toggleReveal(item.id)} className="absolute inset-0 grid place-items-center bg-[#202328]/35 text-white" aria-label="查看 R18 内容">
              ...
              {!isHidden && item.rating === "r18" && (
                <button type="button" onClick={() => toggleReveal(item.id)} className="absolute right-2 top-2 grid h-8 w-8 place-items-center rounded-full bg-[#202328]/70 text-white" aria-label="重新隐藏 R18 内容" title="重新打码">
                  <EyeOff size={15} />
                </button>
              )}
            </div>
            <div className="p-3"><div className="flex items-start justify-between gap-2"><div><h2 className="font-semibold">{item.title}</h2><p className="mt-1 text-xs text-[var(--muted)]">作者：{item.authorName || item.ownerName}</p><p className="mt-0.5 text-[10px] text-[var(--muted)]">上传者：{item.ownerName} · {ratingLabel(item.rating)} · {item.source}</p></div><div className="flex shrink-0 items-center gap-1"><button type="button" onClick={() => share(item)} className="flex items-center gap-1 text-xs text-[var(--muted)] hover:text-[var(--rose)]" title="复制分享链接"><Link2 size={15} /></button><button type="button" onClick={() => copyImageLink(item)} className="flex items-center gap-1 text-xs text-[var(--muted)] hover:text-[var(--rose)]" title="复制图片直链"><ImageIcon size={15} /></button><button type="button" onClick={() => like(item.id)} className="flex items-center gap-1 text-xs text-[var(--rose)]"><Heart size={15} />{item.likes}</button></div></div>
              <div className="mt-3 flex flex-wrap gap-1">{item.tags.map((tag) => <span key={tag} className="rounded bg-[#f1eee7] px-2 py-1 text-[10px]">{tag}</span>)}</div>
              {Object.keys(item.parameters).length > 0 ? <Link href={importHref(item)} className="mt-4 flex h-9 items-center justify-center gap-2 rounded bg-[#292d2c] text-xs font-semibold text-white"><RotateCcw size={14} />导入全部参数</Link> : <p className="mt-4 text-xs text-[var(--muted)]">作者未公开详细参数</p>}
            </div>
          </article>
          );
